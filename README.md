...
- `daily-arxiv` 也支持在用户显式指定单篇论文时跳过筛选，并补齐可信论文入口后直接委派精读。
...
**设置每日定时任务（推荐入口）：**
```text
每天工作日上午 09:00，自动执行「每日 arXiv」任务，结果保存到「日常学习」知识库。
```

**临时手动触发：**
```text
执行今天的「每日 arXiv」任务，结果保存到「日常学习」知识库。
```

**覆盖抓取范围：**
```text
执行今天的「每日 arXiv」任务，使用最近 48 小时窗口，candidate pool 调整到 150，结果保存到「日常学习」知识库。
```

**精读单篇论文：**
```text
精读这篇论文：https://arxiv.org/abs/2504.xxxxx，保存到「日常学习」知识库。
```
## 运行依赖

运行 `daily-arxiv` 的抓取脚本前，先安装 Python 依赖：
