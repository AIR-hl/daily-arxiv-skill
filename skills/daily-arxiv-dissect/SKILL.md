...
2. ⚠ 读取 `assets/templates/paper_template.md` 和 `references/writing-style.md`，严格根据模板结构组织出一篇解读报告。注意 `paper_template.md` 只是内容结构模板，不是飞书中最终的呈现形态。
3. 以"增量声明"为阅读视角贯穿全文进行理解：厘清前人边界、本文增量、内部机制、证据力度与最终判断。文章结构严格按 paper_template.md 的章节组织，而不是按这里的角度直接写作。
...
- HTML 解析必须按结构读取：标题、章节、段落、图注、表格，以及数学节点中的 `alttext` / TeX annotation。
- 禁止把 arXiv HTML 当普通网页做纯文本粗抽；如果关键数字、公式或表格在纯文本模式下丢失，应切换到结构化读取或回退到 PDF / TEX 做核对。
- `html_url=unknown` 表示抓取时未能确认，不等于“确认没有 HTML 版本”；因此允许在子 skill 内只重试一次，再决定是否回退。
